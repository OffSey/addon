...
RegisterNetEvent("rtx_themepark:Global:UsingAttractionPlayer", function(state)
    ...
    for i = 1, #bypass do
        ...
        if not result then
            Error(("[RTX Theme Park] Can't give temporany permission!\nPermission: ^5\"%s.%s\"^0\nPlayer: ^5[%s] %s^0\nReason: ^5%s^0"):format(v.category,v.permission,source,GetPlayerName(source),errorText))
        else
            Debug(("[RTX Theme Park] temporany Permission ^5\"%s.%s\"^0 was %s succesfully!\nPlayer changed: ^5[%s] %s^0"):format(v.category,v.permission,state == true and"^2granted^0" or "^1removed^0",source,GetPlayerName(source)))
        end
    end
end)